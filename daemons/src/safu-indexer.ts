/**
 * SAFU Indexer Daemon — multi-chain sweep
 *
 * Periodically sweeps ALL platform tokens via SafuLens eth_call across
 * every enabled network and writes badge data (is_safu, has_liquidity,
 * lp_burned, etc.) to the DB. The explore page reads these columns for
 * instant SQL filtering and sorting — no client-side RPC needed for
 * the list view.
 *
 * Boots one read-only context per chain (provider, dex factory, weth)
 * and rotates through them on each sweep cycle.
 *
 * Usage:
 *   bun src/safu-indexer.ts
 *
 * Environment:
 *   API_BASE_URL     — Backend URL (e.g. https://tokenkrafter.com)
 *   SYNC_SECRET      — Auth token for backend API
 *   SWEEP_INTERVAL   — Seconds between full sweeps (default: 300 = 5 min)
 *   BATCH_SIZE       — Tokens per SafuLens eth_call (default: 100)
 *   SAFU_BYTECODE    — Path to SafuLens bytecode JSON (optional)
 */

import { ethers } from 'ethers';
import * as fs from 'fs';
import * as path from 'path';
import { createManagedProvider, type ManagedProvider } from '../lib/provider';
import { loadNetworks, pickDaemonRpc, type Network } from './lib/chains';

// ── Config (chain-agnostic) ────────────────────────────────
const SWEEP_INTERVAL = parseInt(process.env.SWEEP_INTERVAL || '300') * 1000;
const BATCH_SIZE = parseInt(process.env.BATCH_SIZE || '100');
const API_BASE = process.env.API_BASE_URL || 'http://localhost:5173';
const SYNC_SECRET = process.env.SYNC_SECRET || '';

if (!SYNC_SECRET) console.warn('⚠️  No SYNC_SECRET — API writes will be rejected');

// ── SafuLens bytecode ─────────────────────────────────────
let SAFU_BYTECODE = '';

async function loadSafuBytecode(): Promise<string> {
	if (SAFU_BYTECODE) return SAFU_BYTECODE;

	const envPath = process.env.SAFU_BYTECODE;
	if (envPath && fs.existsSync(envPath)) {
		const json = JSON.parse(fs.readFileSync(envPath, 'utf-8'));
		SAFU_BYTECODE = json.bytecode || json;
		return SAFU_BYTECODE;
	}

	const candidates = [
		path.resolve(import.meta.dirname || __dirname, '../../artifacts/contracts/simulators/SafuLens.sol/SafuLens.json'),
		path.resolve(import.meta.dirname || __dirname, 'SafuLens.json'),
	];
	for (const p of candidates) {
		try {
			const json = JSON.parse(fs.readFileSync(p, 'utf-8'));
			SAFU_BYTECODE = json.bytecode;
			console.log(`   SafuLens bytecode loaded from: ${p}`);
			return SAFU_BYTECODE;
		} catch {}
	}

	throw new Error('SafuLens bytecode not found. Set SAFU_BYTECODE env or place SafuLens.json next to this script.');
}

// ── Token list from DB ────────────────────────────────────
interface TokenRow { address: string; chain_id: number; }

async function fetchAllTokenAddresses(chainId: number): Promise<string[]> {
	const res = await fetch(`${API_BASE}/api/created-tokens?chain_id=${chainId}&fields=address`, {
		headers: { Authorization: `Bearer ${SYNC_SECRET}` },
	});
	if (!res.ok) {
		console.warn(`   ⚠️ [c${chainId}] Token list fetch failed: ${res.status}`);
		return [];
	}
	const rows: TokenRow[] = await res.json();
	return rows.map((r) => r.address);
}

// ── SafuLens batch call ───────────────────────────────────
interface SafuResult {
	token: string;
	isMintable: boolean;
	isTaxable: boolean;
	isPartner: boolean;
	ownerIsZero: boolean;
	taxCeilingLocked: boolean;
	tradingEnabled: boolean;
	hasLiquidity: boolean;
	lpBurned: boolean;
	lpBurnedPct: number;
	buyTaxBps: number;
	sellTaxBps: number;
	isSafu: boolean;
}

async function querySafuLensBatch(
	provider: ethers.Provider,
	bytecode: string,
	tokenFactory: string,
	dexFactory: string,
	weth: string,
	usdt: string,
	tokenAddrs: string[],
): Promise<SafuResult[]> {
	const abiCoder = ethers.AbiCoder.defaultAbiCoder();
	const args = abiCoder.encode(
		['address', 'address', 'address', 'address', 'address[]'],
		[tokenFactory, dexFactory, weth, usdt, tokenAddrs],
	);
	const callData = bytecode + args.slice(2);
	const raw = await provider.call({ data: callData, gasLimit: 50_000_000 });
	if (!raw || raw === '0x') return [];

	const decoded = abiCoder.decode(
		['tuple(address token, bool isMintable, bool isTaxable, bool isPartner, address owner, bool ownerIsZero, bool taxCeilingLocked, uint256 buyTaxBps, uint256 sellTaxBps, uint256 transferTaxBps, bool tradingEnabled, bool hasLiquidity, bool lpBurned, uint256 lpBurnedPct, bool isSafu)[]'],
		raw,
	);
	return (decoded[0] as any[]).map((s: any) => ({
		token: s.token.toLowerCase(),
		isMintable: s.isMintable,
		isTaxable: s.isTaxable,
		isPartner: s.isPartner,
		ownerIsZero: s.ownerIsZero,
		taxCeilingLocked: s.taxCeilingLocked,
		tradingEnabled: s.tradingEnabled,
		hasLiquidity: s.hasLiquidity,
		lpBurned: s.lpBurned,
		lpBurnedPct: Number(s.lpBurnedPct),
		buyTaxBps: Number(s.buyTaxBps),
		sellTaxBps: Number(s.sellTaxBps),
		isSafu: s.isSafu,
	}));
}

// ── API write ─────────────────────────────────────────────
async function updateSafuBatch(chainId: number, results: SafuResult[]): Promise<number> {
	let updated = 0;
	const batch = results.map((r) => ({
		address: r.token,
		chain_id: chainId,
		is_safu: r.isSafu,
		has_liquidity: r.hasLiquidity,
		lp_burned: r.lpBurned,
		lp_burned_pct: r.lpBurnedPct,
		tax_ceiling_locked: r.taxCeilingLocked,
		owner_renounced: r.ownerIsZero,
		trading_enabled: r.tradingEnabled,
		buy_tax_bps: r.buyTaxBps,
		sell_tax_bps: r.sellTaxBps,
	}));

	try {
		const res = await fetch(`${API_BASE}/api/created-tokens/safu`, {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SYNC_SECRET}` },
			body: JSON.stringify({ tokens: batch }),
		});
		if (res.ok) {
			updated = batch.length;
		} else {
			console.warn(`   ⚠️ [c${chainId}] Batch SAFU update failed: ${res.status}`);
			for (const t of batch) {
				try {
					const r2 = await fetch(`${API_BASE}/api/created-tokens`, {
						method: 'PATCH',
						headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SYNC_SECRET}` },
						body: JSON.stringify(t),
					});
					if (r2.ok) updated++;
				} catch {}
			}
		}
	} catch (e: any) {
		console.error(`   ✗ [c${chainId}] SAFU update error: ${e.message?.slice(0, 80)}`);
	}
	return updated;
}

// ── Per-chain context ─────────────────────────────────────
type ChainCtx = {
	chainId: number;
	name: string;
	managed: ManagedProvider;
	tokenFactory: string;
	usdt: string;
	dexFactory: string;
	weth: string;
};

async function buildChainCtx(net: Network): Promise<ChainCtx | null> {
	const tokenFactory = (net.platform_address as string) || '';
	const usdt = (net.usdt_address as string) || '';
	const dexRouter = (net.dex_router as string) || '';
	if (!tokenFactory || !usdt || !dexRouter) {
		console.warn(`   ⚠️ [c${net.chain_id}] missing platform/usdt/dex_router — skipping`);
		return null;
	}
	const { http, ws } = pickDaemonRpc(net);
	const rpcUrl = http || ws;
	if (!rpcUrl) {
		console.warn(`   ⚠️ [c${net.chain_id}] no rpc — skipping`);
		return null;
	}
	const managed = createManagedProvider({ chainId: net.chain_id, httpRpc: rpcUrl, wsRpc: ws });
	try {
		const dexC = new ethers.Contract(dexRouter, [
			'function factory() view returns (address)',
			'function WETH() view returns (address)',
		], managed.getProvider());
		const [dexFactory, weth] = await Promise.all([dexC.factory(), dexC.WETH()]);
		console.log(`   [c${net.chain_id}/${net.name || 'chain'}] TokenFactory ${tokenFactory.slice(0, 10)}…  DEX ${dexFactory.slice(0, 10)}…`);
		return {
			chainId: net.chain_id,
			name: (net.name as string) || `chain-${net.chain_id}`,
			managed,
			tokenFactory,
			usdt,
			dexFactory,
			weth,
		};
	} catch (e: any) {
		console.error(`   ✗ [c${net.chain_id}] ctx build failed: ${e.message?.slice(0, 80)}`);
		await managed.close().catch(() => {});
		return null;
	}
}

async function sweepChain(c: ChainCtx, bytecode: string): Promise<void> {
	const ts = new Date().toLocaleTimeString();
	const allAddrs = await fetchAllTokenAddresses(c.chainId);
	console.log(`[${ts}] [c${c.chainId}] sweep — ${allAddrs.length} tokens`);
	if (allAddrs.length === 0) return;

	let totalUpdated = 0;
	for (let offset = 0; offset < allAddrs.length; offset += BATCH_SIZE) {
		const batch = allAddrs.slice(offset, offset + BATCH_SIZE);
		try {
			let results;
			try {
				results = await querySafuLensBatch(
					c.managed.getProvider(), bytecode,
					c.tokenFactory, c.dexFactory, c.weth, c.usdt,
					batch,
				);
			} catch (e: any) {
				const msg = String(e?.message ?? '');
				const transient = /destroyed|cancelled|timeout|UNSUPPORTED_OPERATION|ECONN/i.test(msg);
				if (!transient) throw e;
				console.warn(`   ⚠️ [c${c.chainId}] batch ${offset}: WS hiccup (${msg.slice(0, 60)}) — retry once`);
				await new Promise((r) => setTimeout(r, 1500));
				results = await querySafuLensBatch(
					c.managed.getProvider(), bytecode,
					c.tokenFactory, c.dexFactory, c.weth, c.usdt,
					batch,
				);
			}
			if (results.length > 0) {
				const n = await updateSafuBatch(c.chainId, results);
				totalUpdated += n;
			}
			if (offset + BATCH_SIZE < allAddrs.length) await new Promise((r) => setTimeout(r, 200));
		} catch (e: any) {
			console.error(`   ✗ [c${c.chainId}] batch ${offset}-${offset + batch.length}: ${e.message?.slice(0, 80)}`);
		}
	}
	console.log(`   [c${c.chainId}] ✓ updated ${totalUpdated}/${allAddrs.length}`);
}

// ── Main loop ────────────────────────────────────────────
async function main() {
	const bytecode = await loadSafuBytecode();

	console.log(`\n🛡️  SAFU Indexer (multi-chain) starting`);
	console.log(`   API: ${API_BASE}`);
	console.log(`   Sweep interval: ${SWEEP_INTERVAL / 1000}s`);
	console.log(`   Batch size: ${BATCH_SIZE}`);

	let networks: Network[];
	try {
		networks = await loadNetworks(API_BASE, SYNC_SECRET ? `Bearer ${SYNC_SECRET}` : undefined);
	} catch (e: any) {
		console.error(`Failed to load networks: ${e.message}`);
		process.exit(1);
	}
	if (networks.length === 0) {
		console.error('No enabled networks');
		process.exit(1);
	}
	console.log(`   networks: ${networks.map((n) => `${n.chain_id}/${n.name}`).join(', ')}`);

	const chains: ChainCtx[] = [];
	for (const net of networks) {
		const c = await buildChainCtx(net);
		if (c) chains.push(c);
	}
	if (chains.length === 0) {
		console.error('No usable chain contexts');
		process.exit(1);
	}

	let sweepCount = 0;
	let running = true;
	process.on('SIGINT', () => { running = false; console.log('\n⏹  Shutting down...'); });
	process.on('SIGTERM', () => { running = false; });

	while (running) {
		sweepCount++;
		const ts = new Date().toLocaleTimeString();
		console.log(`\n[${ts}] Sweep #${sweepCount} across ${chains.length} chain(s)`);
		for (const c of chains) {
			try {
				await sweepChain(c, bytecode);
			} catch (e: any) {
				console.error(`   ❌ [c${c.chainId}] sweep error: ${e.message?.slice(0, 120)}`);
			}
		}
		await new Promise((resolve) => setTimeout(resolve, SWEEP_INTERVAL));
	}

	for (const c of chains) await c.managed.close().catch(() => {});
	console.log('✅ SAFU Indexer stopped.');
}

main().catch((e) => { console.error('Fatal error:', e); process.exit(1); });
