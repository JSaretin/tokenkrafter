...
export async function loadAllChains(
	...
	for (const net of supportedNetworks) {
		...
		try {
			...
			// We only need the first three fields on the dashboard.
			const [owner, totalTokens, tokenFeeUsdt] = await factory.getState();
...
/** Fetch the 20 most recent tokens from the indexed DB. */
