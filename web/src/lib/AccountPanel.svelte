...
<div class="ap" class:ap-closing={!open} role="dialog" aria-label="Account panel">
	...
	{:else if view === 'receive'}
		<div class="flex-1 p-4 flex flex-col gap-3 overflow-y-auto items-center">
			...
			<!-- Buy-crypto shortcut → /trade?mode=buy.
			     Uses goto() rather than <a href> on purpose: we want the
			     /trade page to load BEHIND the still-open wallet panel so
			     the user sees the on-ramp panel come up while they keep
			     working out of their wallet. close() is intentionally NOT
			     called — unlike the Send / Trade buttons further down the
			     account view, which dismiss the panel because their flow
			     fully takes over the page. -->
			<button
				type="button"
				onclick={() => { goto('/trade?mode=buy'); }}
				class="w-full mt-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-[10px] border border-[rgba(0,210,255,0.2)] cursor-pointer bg-gradient-to-br from-[rgba(0,210,255,0.12)] to-[rgba(59,130,246,0.12)] text-[#00d2ff] font-mono text-3xs font-bold transition-all duration-150 hover:from-[rgba(0,210,255,0.2)] hover:to-[rgba(59,130,246,0.2)] hover:border-[rgba(0,210,255,0.35)]"
			>
