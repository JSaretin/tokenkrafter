	...
	<div class="overflow-y-auto flex-1 min-h-0 px-2 pb-2">
		{#if showImportRow}
			...
			{:else if pastedTokenMeta}
				...
				>
					{#snippet rightSlot()}
						<span class="font-mono text-3xs font-bold text-[#a78bfa] bg-[rgba(139,92,246,0.1)] border border-[rgba(139,92,246,0.2)] py-[3px] px-2 rounded-md shrink-0">{$t('trade.import')}</span>
					{/snippet}
				</TokenListItem>
			{:else}
				<TokenListItem
