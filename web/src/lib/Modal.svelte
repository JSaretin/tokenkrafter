...
<FixedOverlay bind:show {onclose} {mobileSheet} {swipeToDismiss}>
	<div
		class={"w-full " + maxWidth + " bg-(--bg) border border-(--border) rounded-[20px] overflow-hidden shadow-[0_24px_80px_rgba(0,0,0,0.5)] animate-modal-in " + (mobileSheet ? "max-sm:max-w-full max-sm:rounded-t-[20px] max-sm:rounded-b-none max-sm:h-[80vh] max-sm:flex max-sm:flex-col" : "")}
	>
		{#if mobileSheet}
