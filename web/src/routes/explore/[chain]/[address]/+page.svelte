...
<svelte:head>
	...
	<meta property="og:description" content="{view.name} ({view.symbol}) on {chainInfo.name}. {view.description?.slice(0, 120) || $t('explore.detail.metaDescDefault')}" />
	<meta property="og:url" content="https://tokenkrafter.com/explore/{data.chainSlug}/{tokenAddress}" />
	<meta property="og:image" content="/api/og/{data.chainSlug}/{tokenAddress}" />
	<meta property="og:image:width" content="1200" />
	<meta property="og:image:height" content="630" />
	<meta property="og:image:type" content="image/svg+xml" />
	<meta name="twitter:card" content="summary_large_image" />
	<meta name="twitter:title" content="{view.name} ({view.symbol}) | TokenKrafter" />
	<meta name="twitter:description" content="{view.name} ({view.symbol}) on {chainInfo.name}." />
	<meta name="twitter:image" content="/api/og/{data.chainSlug}/{tokenAddress}" />
</svelte:head>
...
<div class="max-w-180 mx-auto px-5 pt-8 pb-20 relative">
	...
	<!-- Secondary action row — Wallet / Explorer / Manage. Trade + Share live in
	     the hero now; this strip stays small and tertiary. -->
	<div class="flex items-stretch gap-1.5 mb-5">
		<button class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-(--border) bg-(--bg-surface) text-(--text-dim) font-mono text-3xs cursor-pointer transition-all duration-150 hover:bg-(--bg-surface-hover) hover:text-(--text) hover:border-(--border-input)" onclick={addToWallet} aria-label={$t('explore.detail.wallet')}>
			...
			<span>{addedToWallet ? $t('explore.detail.added') : $t('explore.detail.wallet')}</span>
		</button>
		<a href="{chainInfo.explorer}/token/{tokenAddress}" target="_blank" rel="noopener" class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-(--border) bg-(--bg-surface) text-(--text-dim) font-mono text-3xs cursor-pointer transition-all duration-150 no-underline hover:bg-(--bg-surface-hover) hover:text-(--text) hover:border-(--border-input)" aria-label={$t('explore.detail.explorer')}>
			<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
			<span>{$t('explore.detail.explorer')}</span>
		</a>
		{#if isCreator && view.isOnPlatform}
			<a href="/manage-tokens/{data.chainSlug}/{tokenAddress}" class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-[rgba(168,85,247,0.1)] border border-[rgba(168,85,247,0.22)] text-[#a855f7] syne font-bold text-3xs cursor-pointer transition-all duration-150 no-underline hover:bg-[rgba(168,85,247,0.16)]" aria-label={$t('explore.detail.manage')}>
			...
			<span>{$t('explore.detail.manage')}</span>
			</a>
		{/if}
	</div>

	<!-- Technical details — collapsed by default. Most visitors don't need
	     tax-simulation / protection windows / contract info; the creator
	     does. Components inside still render conditionally on their own
	     internal gates (TaxPanel only shows when isTaxable, etc.) so
	     non-platform tokens don't see empty cards. -->
	<section class="mb-6">
		<button
			type="button"
			class="flex items-center gap-2 w-full px-4 py-3 rounded-xl border border-(--border-subtle) bg-(--bg-surface) text-left cursor-pointer transition-all duration-150 hover:border-(--border-input) hover:bg-(--bg-surface-hover)"
			onclick={() => showTechnical = !showTechnical}
			aria-expanded={showTechnical}
		>
			<svg class="text-(--text-dim) shrink-0" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
			<span class="syne text-xs2 font-bold text-(--text-heading) uppercase tracking-wide flex-1">{$t('explore.detail.technicalDetails')}</span>
			<span class="font-mono text-3xs text-(--text-dim) hidden sm:inline">{$t('explore.detail.technicalHint')}</span>
			<svg class={'text-(--text-dim) shrink-0 transition-transform duration-150 ' + (showTechnical ? 'rotate-180' : '')} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
		</button>
