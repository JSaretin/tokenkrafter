...
<div class="max-w-[800px] mx-auto px-4 pt-6 pb-[60px]">
	<div class="mb-8">
		<h1 class="heading-1 mb-1.5">Contracts</h1>
		<p class="font-mono text-13 text-muted m-0">All smart contracts are verified and publicly accessible on the blockchain.</p>
	</div>
	...
	{#each networks as network}
		<div class="mb-8">
			<div class="inline-flex items-center gap-2 mb-3.5">
				...
				<span class="font-mono text-xs2 text-dim bg-surface px-2 py-0.5 rounded">Chain ID: {network.chain_id}</span>
			</div>
					...
					{#if addr && addr !== '' && addr !== '0x'}
						<div class="bg-surface border border-line rounded-xl px-4 py-3.5">
							<div class="flex justify-between items-center mb-1">
								<span class="font-display text-13 font-bold text-heading">{info.name}</span>
								{#if key === 'dex_router' || key === 'usdt_address' || key === 'usdc_address'}
									<span class="font-mono text-xs4 text-dim bg-surface-hover px-1.5 py-0.5 rounded">Third-party</span>
								{:else}
									<span class="font-mono text-xs4 text-[#00d2ff] bg-[rgba(0,210,255,0.08)] px-1.5 py-0.5 rounded">TokenKrafter</span>
								{/if}
							</div>
							<p class="font-mono text-3xs text-muted m-0 mb-2.5 leading-[1.5]">{info.desc}</p>
							<div class="font-mono text-xs2 text-[#00d2ff] bg-surface rounded-lg px-2.5 py-2 break-all leading-[1.6] mb-2.5 border border-[rgba(0,210,255,0.08)]">{addr}</div>
							<div class="flex gap-1.5">
								<button class="inline-flex items-center gap-1 font-mono text-xs2 px-3 py-1.5 rounded-md border border-line bg-surface text-muted cursor-pointer transition-all duration-150 no-underline hover:border-[rgba(0,210,255,0.3)] hover:text-[#00d2ff]" onclick={() => copyAddr(addr)}>
									{#if copied === addr}
								...
								<a class="inline-flex items-center gap-1 font-mono text-xs2 px-3 py-1.5 rounded-md border border-line bg-surface text-muted cursor-pointer transition-all duration-150 no-underline hover:border-[rgba(16,185,129,0.3)] hover:text-emerald-500" href={explorerLink(network, addr)} target="_blank" rel="noopener">
