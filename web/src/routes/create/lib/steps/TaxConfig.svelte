...
<div class="flex flex-col gap-4">
	<!-- Tax ceiling info -->
	<div class="py-[10px] px-3 rounded-lg bg-[rgba(248,113,113,0.06)] border border-[rgba(248,113,113,0.22)] text-[#fca5a5] text-xs2 font-mono leading-[1.55]">
		...
		<div class="mt-[6px] text-3xs text-[#fca5a5] opacity-90 [&_strong]:text-[#fecaca] [&_strong]:font-bold">
	...
	<div class="flex flex-col gap-2">
		...
		<div class={'tc-rate flex items-center justify-between gap-3 py-3 px-[14px] max-[500px]:py-[10px] max-[500px]:px-3 rounded-[10px] bg-surface border ' + (buyOver ? 'border-[rgba(248,113,113,0.25)]' : 'border-line-subtle')}>
			<div class="flex items-center gap-2 shrink-0">
				...
				<span class="font-display text-13 font-semibold text-foreground">Buy Tax</span>
				<span class={'text-xs4 font-mono ml-auto uppercase tracking-[0.04em] ' + (buyOver ? 'text-[#f87171]' : 'text-dim')}>max {MAX_BUY}%</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="tc-presets flex gap-[3px]">
					{#each presets as p}
						<button type="button" class={'py-[3px] px-2 rounded-md border bg-transparent font-mono text-xs4 cursor-pointer transition-all ' + (num(buyTaxPct) === p ? 'text-brand-cyan border-[rgba(0,210,255,0.3)] bg-[rgba(0,210,255,0.06)]' : 'text-dim border-line-subtle hover:text-muted hover:border-line-input')} onclick={() => buyTaxPct = String(p)}>{p}%</button>
					{/each}
				</div>
				<div class={'flex items-center gap-px w-14 py-1 px-2 rounded-md bg-surface-input border ' + (buyOver ? 'border-[rgba(248,113,113,0.4)]' : 'border-line')}>
					...
					<span class="font-mono text-3xs text-dim">%</span>
				</div>
			</div>
		</div>
		...
		<div class={'tc-rate flex items-center justify-between gap-3 py-3 px-[14px] max-[500px]:py-[10px] max-[500px]:px-3 rounded-[10px] bg-surface border ' + (sellOver ? 'border-[rgba(248,113,113,0.25)]' : 'border-line-subtle')}>
			<div class="flex items-center gap-2 shrink-0">
				...
				<span class="font-display text-13 font-semibold text-foreground">Sell Tax</span>
				<span class={'text-xs4 font-mono ml-auto uppercase tracking-[0.04em] ' + (sellOver ? 'text-[#f87171]' : 'text-dim')}>max {MAX_SELL}%</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="tc-presets flex gap-[3px]">
					{#each presets as p}
						<button type="button" class={'py-[3px] px-2 rounded-md border bg-transparent font-mono text-xs4 cursor-pointer transition-all ' + (num(sellTaxPct) === p ? 'text-brand-cyan border-[rgba(0,210,255,0.3)] bg-[rgba(0,210,255,0.06)]' : 'text-dim border-line-subtle hover:text-muted hover:border-line-input')} onclick={() => sellTaxPct = String(p)}>{p}%</button>
					{/each}
				</div>
		...
		<div class={'tc-rate flex items-center justify-between gap-3 py-3 px-[14px] max-[500px]:py-[10px] max-[500px]:px-3 rounded-[10px] bg-surface border ' + (transferOver ? 'border-[rgba(248,113,113,0.25)]' : 'border-line-subtle')}>
			<div class="flex items-center gap-2 shrink-0">
				...
				<span class="font-display text-13 font-semibold text-foreground">Transfer Tax</span>
				<span class={'text-xs4 font-mono ml-auto uppercase tracking-[0.04em] ' + (transferOver ? 'text-[#f87171]' : 'text-dim')}>max {MAX_TRANSFER}%</span>
			</div>
			<div class="flex items-center gap-2">
				<div class="tc-presets flex gap-[3px]">
					{#each presets.filter(p => p <= MAX_TRANSFER) as p}
						<button type="button" class={'py-[3px] px-2 rounded-md border bg-transparent font-mono text-xs4 cursor-pointer transition-all ' + (num(transferTaxPct) === p ? 'text-brand-cyan border-[rgba(0,210,255,0.3)] bg-[rgba(0,210,255,0.06)]' : 'text-dim border-line-subtle hover:text-muted hover:border-line-input')} onclick={() => transferTaxPct = String(p)}>{p}%</button>
					{/each}
				</div>
	...
	{#if totalTax > 0}
		<div class="px-[2px]">
			...
			<div class="flex items-center gap-2 mt-1">
				<span class={'font-mono text-3xs ' + ((totalTax > MAX_TOTAL || buyOver || sellOver || transferOver) ? 'text-[#f87171]' : 'text-dim')}>{totalTax}% total</span>
				{#if totalTax > MAX_TOTAL}<span class="text-xs4 text-[#f87171] font-mono">Max {MAX_TOTAL}%</span>{/if}
			</div>
		</div>
	{/if}
	...
	{#if totalTax > 0}
		<div class="flex flex-col gap-2">
			<div class="flex items-baseline gap-2">
				<span class="font-display text-13 font-bold text-muted">Revenue Wallets</span>
				<span class="text-3xs text-dim font-mono">Where collected tax goes</span>
			</div>
			{#each taxWallets as wallet, i}
				<div class="tc-wallet flex items-center gap-[6px] py-2 px-[10px] max-[500px]:flex-wrap rounded-lg bg-surface border border-line-subtle">
					<input class="tc-wallet-addr flex-1 min-w-0 bg-transparent border-none outline-none font-mono text-xs2 max-[500px]:basis-full max-[500px]:text-3xs text-foreground p-0 placeholder:text-dim" bind:value={wallet.address} placeholder="0x... wallet address" />
					<div class="flex items-center gap-px w-[52px] shrink-0 py-[2px] px-[6px] rounded bg-surface-input">
						...
						<span class="text-3xs text-dim font-mono">%</span>
					</div>
			...
			<div class="flex items-center gap-[10px]">
				{#if taxWallets.length < 10}
					<button type="button" class="py-[6px] px-3 rounded-md border border-dashed border-line bg-transparent text-dim font-mono text-3xs cursor-pointer transition-all hover:text-brand-cyan hover:border-[rgba(0,210,255,0.2)]" onclick={addWallet}>+ Add wallet</button>
				{/if}
				{#if totalShares > 100}
					<span class="text-3xs text-[#f87171] font-mono">Shares total {totalShares}% — max 100%</span>
				{:else if sharesBurned > 0 && totalShares > 0}
					<span class="text-3xs text-dim font-mono">{sharesBurned}% will be burned</span>
				{/if}
			</div>
			{#if taxWithoutWallet}
				<div class="mt-[10px] py-2 px-[10px] rounded-lg bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.2)] text-[#fbbf24] text-xs2 font-mono leading-[1.5]">
