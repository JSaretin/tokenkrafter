...
<div class="flex gap-1 mb-2.5">
	{#each FILTERS as f}
		...
		<button
			type="button"
			class={"py-1 px-2.5 rounded-full border font-mono text-xs2 cursor-pointer transition-all duration-[120ms] capitalize " + (filter === f ? "text-cyan border-[rgba(0,210,255,0.25)] bg-[rgba(0,210,255,0.06)]" : "text-[var(--text-dim)] border-[var(--border)] bg-transparent hover:text-[var(--text-muted)] hover:border-[var(--border-input)]")}
			onclick={() => filter = f}
		>
			{labelFor(f)}
			{#if count > 0}
				<span class="inline-flex items-center justify-center min-w-4 h-4 py-0 px-1 rounded-lg text-xxs font-bold bg-[rgba(0,210,255,0.1)] text-cyan ml-[3px]">{count}</span>
			{/if}
		</button>
	{/each}
</div>
