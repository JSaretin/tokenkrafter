	...
	<div class="bg-[rgba(16,185,129,0.05)] border border-[rgba(16,185,129,0.1)] rounded-[10px] p-3.5 my-3 mb-2.5">
		{#if fiatEquivalent}
			<div class="text-center">
				<span class="block font-[Rajdhani,sans-serif] text-2xl2 font-bold text-success leading-tight tabular-nums">{fiatEquivalent}</span>
				<span class="block font-mono text-xs2 text-(--text-dim) mt-1">≈${netUsd} USD</span>
			</div>
		{/if}
		{#if previewNet > 0n}
			<div class={"flex flex-col gap-1 " + (fiatEquivalent ? "mt-2 pt-2 border-t border-(--border-subtle)" : "")}>
				<div class="flex justify-between font-mono text-xs2 text-(--text-dim)">
					<span>{$t('trade.feeWithPct').replace('{pct}', feePct)}</span>
					<span>${feeUsd}</span>
				</div>
				<div class="flex justify-between font-mono text-xs2 text-(--text-dim)">
					<span>{$t('trade.youReceive')}</span>
					<span class="text-success font-bold">${netUsd}</span>
				</div>
			</div>
		{/if}
	</div>
