...
<Modal bind:show maxWidth="max-w-[420px]">
	<div class="flex flex-col max-h-[85vh] max-sm:max-h-[90vh]">
	<div class="flex justify-between items-center py-4 px-5 border-b border-(--border)">
		<h3 class="syne text-[16px] font-bold text-(--text-heading) m-0">{$t('trade.selectAToken')}</h3>
	...
	<div class="overflow-y-auto flex-1 min-h-0 px-2 pb-2">
		{#if showImportRow}
			...
			{:else if pastedTokenMeta}
				...
				>
					{#snippet rightSlot()}
						<span class="font-mono text-xs2 font-bold text-purple bg-[rgba(139,92,246,0.1)] border border-[rgba(139,92,246,0.2)] py-[3px] px-2 rounded-md shrink-0">{$t('trade.import')}</span>
					{/snippet}
				</TokenListItem>
			{:else}
				<TokenListItem
		...
		{#if dbSearchLoading}
			<div class="flex items-center justify-center py-2.5 px-3 opacity-50">
				<span class="font-mono text-xs2 text-(--text-muted)">{$t('trade.searching')}</span>
			</div>
