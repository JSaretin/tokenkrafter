...
<FixedOverlay show={true} onclose={onClose}>
	<div class="w-full max-w-[420px] bg-(--bg) border border-(--border) rounded-[20px] overflow-hidden max-h-[70vh] max-sm:h-[80vh] max-sm:max-h-[80vh] max-sm:max-w-full max-sm:rounded-b-none flex flex-col">
		...
		<div class="px-2 pb-2 overflow-y-auto flex-1 [scrollbar-width:thin] [scrollbar-color:var(--bg-surface-hover)_transparent]">
			{#each filtered as bank}
				<button class="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl border-0 bg-transparent cursor-pointer transition-all duration-150 text-left hover:bg-(--bg-surface-hover)" onclick={() => onSelect(bank)}>
					<div class="w-8 h-8 rounded-full shrink-0 flex items-center justify-center bg-[rgba(16,185,129,0.08)] text-[#10b981] border border-[rgba(16,185,129,0.15)] font-[Syne,sans-serif] text-13 font-bold">{bank.name.charAt(0)}</div>
					<span class="font-mono text-13 text-(--text)">{bank.name}</span>
				</button>
			{/each}
