...
<FixedOverlay show={true} onclose={onClose}>
	<div class="w-full max-w-105 bg-(--bg) border border-(--border) rounded-[20px] overflow-hidden max-h-[85vh] max-sm:max-h-[90vh] max-sm:max-w-full max-sm:rounded-t-[20px] max-sm:rounded-b-none flex flex-col">
		<div class="flex justify-between items-center px-5 py-4 border-b border-(--border)">
			<h3 class="font-[Syne,sans-serif] text-base font-bold text-(--text-heading) m-0">{$t('trade.selectBank')}</h3>
		...
		<div class="px-2 pb-2 overflow-y-auto flex-1 [scrollbar-width:thin] [scrollbar-color:var(--bg-surface-hover)_transparent]">
			{#each filtered as bank}
				<button class="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl border-0 bg-transparent cursor-pointer transition-all duration-150 text-left hover:bg-(--bg-surface-hover)" onclick={() => onSelect(bank)}>
					<div class="w-8 h-8 rounded-full shrink-0 flex items-center justify-center bg-[rgba(16,185,129,0.08)] text-success border border-[rgba(16,185,129,0.15)] font-[Syne,sans-serif] text-sm2 font-bold">{bank.name.charAt(0)}</div>
					<span class="font-mono text-sm2 text-(--text)">{bank.name}</span>
				</button>
			{/each}

			{#if filtered.length === 0}
				<p class="text-center p-5 text-(--text-muted) font-mono text-xs">{$t('trade.noBanksFound')}</p>
			{/if}
		</div>
	</div>
</FixedOverlay>
