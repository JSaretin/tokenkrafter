<script lang="ts">
	...
	const btnCls = 'flex items-center justify-center w-[22px] h-[22px] rounded-[5px] border-0 bg-(--bg-surface-input) text-(--text-dim) cursor-pointer transition-all duration-150 no-underline hover:bg-[rgba(0,210,255,0.1)] hover:text-cyan';
</script>

<div class="flex items-center gap-1 shrink-0">
	<span class="font-mono text-xs2 text-(--text-dim)">{short}</span>
	{#if oncopy}
