...
{#if isHoneypot}
	<div class="inline-flex items-center gap-[5px] mt-2 font-mono text-[10px] text-[#f87171] bg-[rgba(239,68,68,0.08)] border border-[rgba(239,68,68,0.15)] rounded-md py-1 px-2 font-bold">
		<span class="w-[5px] h-[5px] rounded-full shrink-0 bg-[#f87171]"></span>
		{$t('trade.honeypot')}
	</div>
{:else if showTax}
	<div class="inline-flex items-center gap-[5px] mt-2 font-mono text-[10px] text-[#f59e0b] bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.15)] rounded-md py-1 px-2">
	...
	{#if totalTax > 0}
		<p class="font-mono text-[10px] text-[#fbbf24] mt-1.5 mb-0">{$t('trade.taxDeductedNote').replace('{pct}', String(totalTax))}</p>
	{/if}
{/if}
